...
class WaypointFollowing
{
    public:
        ...
        void getPosition()
        {
            try
            {
                transformStamped = tfBuffer.lookupTransform("world", "base_link", ros::Time::now(), ros::Duration(3.0));
                // "map" 프레임에서 "base_link" 프레임으로의 변환을 가져오려함.
            }
        ...
        void publishGoal()
        {
            if (obs_flag) // 장애물 없는 경우 (안전)
            {
                ...
                carrot.target_pose.header.frame_id = "world";
                carrot.target_pose.header.stamp = ros::Time::now();
            
                carrot_point.header.frame_id = "world";
